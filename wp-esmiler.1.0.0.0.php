...
Description: WP eSmiler is an emoticon plug-in for WordPress platform, created by Phong Thai Cao, from JavaScriptBank.com. This smiley plug-in is super easy to use and setup, update new emoticon sets by one click.
Version: 1.0.0.0
...
?>
			...
			<script type="text/javascript">
				/*
					...
					jsB@nk.com @ www.JavaScriptBank.com - all your JavaScript problems should be solved
				*/
				jQuery('.icons-listing tr').each(function(i){
					jQuery(this).click(function(){
						jQuery(':checkbox', this).attr('checked', !jQuery(':checkbox', this).attr('checked'));
						//alert(jQuery(':checkbox', this).attr('checked'));
					});
				});
				...
				jQuery('.off-groups').click(function (){
					jQuery(':checkbox', jQuery(this).parent().parent()).attr('checked', !jQuery(jQuery(':checkbox', jQuery(this).parent().parent())[0]).attr('checked'));
				});
				jQuery('.wp-esmiler-notes').click(function() {
					jQuery(':checkbox[name^=' + jQuery(this).attr('rel') + ']').attr('checked', !jQuery(jQuery(':checkbox[name^=' + jQuery(this).attr('rel') + ']')[0]).attr('checked'));
					//alert(jQuery(':checkbox[name^=' + jQuery(this).attr('rel') + ']').length);
				});
			</script>
...
<script language="javascript">
	...
	*/
	jQuery(document).ready(function(){
		jQuery('#wp-esmiler-icons-toggle').click(function(){
			jQuery('#wp-esmiler-emoticon-list').slideToggle();
		});
		jQuery('.wp-esmiler-icon').click(function(){
			//alert(jQuery('#comment').attr('selectionStart'));
			var cmt = jQuery('#comment').val(),
				start = jQuery('#comment').attr('selectionStart'),
				end = jQuery('#comment').attr('selectionEnd');
			
			jQuery('#comment').val( cmt.substring(0, start) + " " + jQuery(this).attr('title') + " " + cmt.substring(end, cmt.length) );
		});
	});
</script>
<?php
